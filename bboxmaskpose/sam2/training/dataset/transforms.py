...
def hflip(datapoint, index):
    ...
    for obj in datapoint.frames[index].objects:
        if obj.segment is not None:
            obj.segment[0] = F.hflip(obj.segment[0])
...
def resize(datapoint, index, size, max_size=None, square=False, v2=False):
    ...
    for obj in datapoint.frames[index].objects:
        if obj.segment is not None:
            obj.segment[0] = F.resize(obj.segment[0][None, None], size).squeeze()
...
def pad(datapoint, index, padding, v2=False):
    ...
    for obj in datapoint.frames[index].objects:
        if obj.segment is not None:
            if v2:
                if len(padding) == 2:
                    obj.segment[0] = Fv2.pad(obj.segment[0], (0, 0, padding[0], padding[1]))
                else:
                    obj.segment[0] = Fv2.pad(obj.segment[0], tuple(padding))
            else:
                if len(padding) == 2:
                    obj.segment[0] = F.pad(obj.segment[0], (0, 0, padding[0], padding[1]))
                else:
                    obj.segment[0] = F.pad(obj.segment[0], tuple(padding))
    return datapoint
...
class RandomAffine:
    ...
    def __call__(self, datapoint: VideoDatapoint, **kwargs):
        ...
        if self.log_warning:
            logging.warning(f"Skip RandomAffine for zero-area mask in first frame after {self.num_tentatives} tentatives")
        return datapoint
    ...
    def transform_datapoint(self, datapoint: VideoDatapoint):
        ...
        for img_idx, img in enumerate(datapoint.frames):
            this_masks = [obj.segment[0].unsqueeze(0) if obj.segment is not None else None for obj in img.objects]
            if not self.consistent_transform:
            ...
            for i in range(len(img.objects)):
                img.objects[i].segment[0] = transformed_masks[i]
...
def random_mosaic_frame(
    ...
    for obj in datapoint.frames[index].objects:
        if obj.segment is None:
            continue
        assert obj.segment[0].shape == (H_im, W_im) and obj.segment[0].dtype == torch.uint8
        segment_output = torch.zeros_like(obj.segment[0])
