...
from training.dataset.vos_segment_loader import JSONSegmentLoader, MultiplePNGSegmentLoader, PalettisedPNGSegmentLoader, SA1BSegmentLoader
...
class SA1BRawDataset(VOSRawDataset):
    def __init__(
        ...
        print("file list txt", file_list_txt)
        # Read the subset defined in file_list_txt
        if file_list_txt is not None:
            with g_pathmgr.open(file_list_txt, "r") as f:
                subset = [os.path.splitext(line.strip())[0] for line in f]
