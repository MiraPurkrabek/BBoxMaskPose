...
class LazySegments:
    ...
    def __getitem__(self, key):
        ...
        rle = self.segments[key][0]
        mask = torch.from_numpy(mask_utils.decode([rle])).permute(2, 0, 1)[0]
        self.cache[key] = [mask, self.segments[key][1], self.segments[key][2]]
        return self.cache[key]
...
class SA1BSegmentLoader:
    def __init__(
        ...
        width_coeff = 1024.0 / float(self.frame_annots["image"]["width"])
        height_coeff = 1024.0 / float(self.frame_annots["image"]["height"])
        ...
        for frame_annot in self.frame_annots:
            ...
            # # ADD BBOX:
            # frame_annot["point_coords"].append(frame_annot["bbox"][0:2])
            # frame_annot["point_coords"].append(frame_annot["bbox"][2:4])
            # frame_annot["point_labels"].extend([2, 3])

            pt_coords = torch.tensor(frame_annot["point_coords"], device="cuda:0").float()
            pt_labels = torch.tensor(frame_annot["point_labels"], device="cuda:0").float()
            pt_coords[:, 0] *= width_coeff
            pt_coords[:, 1] *= height_coeff
        ...
        for i in range(len(rle_masks)):
            self.segments[i] = [rle_masks[i], points[i], labels[i]]

    def load(self, frame_idx):
        # print("vos segment loader / segments", self.segments[0])
