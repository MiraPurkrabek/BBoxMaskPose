...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        freeze_image_encoder=True,  # CHNAGED
        **kwargs,
    ):
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        # }
        gt_masks_per_frame = {stage_id: masks.unsqueeze(1) for stage_id, masks in enumerate(input.masks)}  # [B, 1, H_im, W_im]
        ...
        for t in init_cond_frames:
            if not use_pt_input:
                ...
                # find the smallest number of keypoints
                min_k = min(p.shape[0] for p in pts)

                # trim all tensors to this size
                pts_trimmed = [p[:min_k] for p in pts]

                pts_trimmed = [p.to(gt_masks_per_frame[t].device)[:min_k] for p in pts]
                # now stack
                point_coords = torch.stack(pts_trimmed, dim=0)  # [B, min_k, 2]
                labels_trimmed = [l.to(gt_masks_per_frame[t].device)[:min_k] for l in input.labels[t]]
                point_labels = torch.stack(labels_trimmed, dim=0)  # [B, min_k]
                pts_from_gt, labels_from_gt = get_next_point(
                    gt_masks=gt_masks_per_frame[t],
                    pred_masks=None,
                    method=("uniform_or_kpt" if self.training else self.pt_sampling_for_eval),
                    points=point_coords,
                )
                # append the GT points & label
                # expand dims to match batch if necessary
                if pts_from_gt.dim() == 2:
                    pts_from_gt = pts_from_gt.unsqueeze(0)  # [1, N_gt, 2]
                    labels_from_gt = labels_from_gt.unsqueeze(0)  # [1, N_gt]

                point_coords = torch.cat([point_coords, pts_from_gt], dim=1)  # concatenate along keypoint dimension
                point_labels = torch.cat([point_labels, labels_from_gt], dim=1)  # same for labels
        ...
        if not use_pt_input:
            ...
            frames_to_add_correction_pt = (
                init_cond_frames + self.rng.choice(backbone_out["frames_not_in_init_cond"], extra_num, replace=False).tolist()
            )
    ...
    def track_step(
        ...
        n_pose_kpts = point_inputs["n_pose_kpts"]

        # comment this section out in case the keypoints are pre-sorted
        _, indices = torch.sort(point_labels)
        point_coords = point_coords[indices]  # comment out in case
        point_labels = point_labels[indices]

        point_labels.fill_(1)
        cropped_point_inputs = {k: (v.clone().to(device) if torch.is_tensor(v) else v) for k, v in point_inputs.items()}
        cropped_point_inputs["point_coords"] = point_coords[:, :n_kpts]
        cropped_point_inputs["point_labels"] = point_labels[:, :n_kpts]
        ...
        current_out["multistep_point_inputs"] = [cropped_point_inputs]
    ...
    def _iter_correct_pt_sampling(
        ...
        for correction_i in range(self.num_correction_pt_per_frame):
            ...
            new_points, new_labels = get_next_point(
                ...
                method="uniform_or_kpt" if self.training else self.pt_sampling_for_eval,
                points=all_pose_kpts["point_coords"][:, :n_pose_kpts],
            )
