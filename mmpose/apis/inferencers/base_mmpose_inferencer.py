...
class BaseMMPoseInferencer(BaseInferencer):
    ...
    def _init_detector(
        ...
        object_type = DATASETS.get(self.cfg.dataset_type).__module__.split("datasets.")[-1].split(".")[0].lower()

        if det_model in ("whole_image", "whole-image") or (det_model is None and object_type not in default_det_models):
            self.detector = None
            ...
            if det_model is None:
                ...
                det_model, det_weights, det_cat_ids = det_info["model"], det_info["weights"], det_info["cat_ids"]
            ...
            if has_mmdet:
                ...
                raise RuntimeError("MMDetection (v3.0.0 or above) is required to build " "inferencers for top-down pose estimation models.")
    ...
    def _load_weights_to_model(self, model: nn.Module, checkpoint: Optional[dict], cfg: Optional[ConfigType]) -> None:
        ...
        if checkpoint is not None:
            ...
            if "dataset_meta" in checkpoint_meta:
                ...
                print_log(
                    "dataset_meta are not saved in the checkpoint's " "meta data, load via config.", logger="current", level=logging.WARNING
                )
            ...
            if hasattr(backend, "isdir") and isdir(inputs):
                ...
                if input_type == "video":
                    ...
                    self.video_info = dict(
                        fps=video.fps, name=os.path.basename(inputs), writer=None, width=video.width, height=video.height, predictions=[]
                    )
                    inputs = video
                    ...
                    raise ValueError(
                        f"Expected input to be an image, video, " f"or folder, but received {inputs} of " f"type {input_type}."
                    )
    ...
    def _get_webcam_inputs(self, inputs: str) -> Generator:
        ...
        inputs = inputs.lower()
        assert inputs.startswith("webcam"), f"Expected input to start with " f'"webcam", but got "{inputs}"'
        ...
        if len(inputs_) == 1:
            ...
            raise ValueError(f'Expected webcam input to have format "webcam:id", ' f'but got "{inputs}"')

        # Attempt to open the video capture object.
        vcap = cv2.VideoCapture(camera_id)
        if not vcap.isOpened():
            print_log(f"Cannot open camera (ID={camera_id})", logger="current", level=logging.WARNING)
            return []
        ...
        self.video_info = dict(fps=fps, name="webcam.mp4", writer=None, width=width, height=height, predictions=[])
    ...
    def preprocess(
        self, inputs: InputsType, batch_size: int = 1, bboxes: Optional[List] = None, bbox_thr: float = 0.3, nms_thr: float = 0.3, **kwargs
    ):
        ...
        for i, input in enumerate(inputs):
            ...
            data_infos = self.preprocess_single(input, index=i, bboxes=bbox, bbox_thr=bbox_thr, nms_thr=nms_thr, **kwargs)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not self._video_input and len(inputs) > 0:
            ...
            if vis_out_dir is not None:
                ...
                assert not file_extension, (
                    f"the argument `vis_out_dir` " f"should be a folder while the input contains multiple " f"images, but got {vis_out_dir}"
                )
        ...
        for proc_inputs, ori_inputs in track(inputs, description="Inference") if self.show_progress else inputs:
            ...
            results = self.postprocess(preds, visualization, return_datasamples=return_datasamples, **postprocess_kwargs)
            yield results
        ...
        **kwargs,
        ...
        for single_input, pred in zip(inputs, preds):
            ...
            visualization = self.visualizer.add_datasample(
                window_name, img, pred, draw_gt=False, draw_bbox=draw_bbox, show=show, wait_time=wait_time, kpt_thr=kpt_thr, **kwargs
            )
        ...
        if self._video_input:
            ...
            if self.video_info["writer"] is None:
                ...
                self.video_info["writer"] = cv2.VideoWriter(
                    out_file, fourcc, self.video_info["fps"], (visualization.shape[1], visualization.shape[0])
                )
            ...
            print_log(f"the output image has been saved at {out_file}", logger="current", level=logging.INFO)
        ...
        if pred_out_dir != "":
            for pred, data_sample in zip(result_dict["predictions"], preds):
                if self._video_input:
                    ...
                    fname = os.path.splitext(os.path.basename(data_sample.metainfo["img_path"]))[0] + ".json"
    ...
    def _finalize_video_processing(
        ...
        if self.video_info["writer"] is not None:
            ...
            print_log(f"the output video has been saved at {out_file}", logger="current", level=logging.INFO)
            self.video_info["writer"].release()

        # Save predictions
        if pred_out_dir:
            fname = os.path.splitext(os.path.basename(self.video_info["name"]))[0] + ".json"
            predictions = [dict(frame_id=i, instances=pred) for i, pred in enumerate(self.video_info["predictions"])]
