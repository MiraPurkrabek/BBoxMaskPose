...
class Pose2DInferencer(BaseMMPoseInferencer):
    ...
    def update_model_visualizer_settings(self, draw_heatmap: bool = False, skeleton_style: str = "mmpose", **kwargs) -> None:
        ...
        if skeleton_style not in ["mmpose", "openpose"]:
            raise ValueError("`skeleton_style` must be either 'mmpose' " "or 'openpose'")
    ...
    @torch.no_grad()
    def forward(self, inputs: Union[dict, tuple], merge_results: bool = True, bbox_thr: float = -1, pose_based_nms: bool = False):
        ...
        if bbox_thr > 0:
            for ds in data_samples:
                if "bbox_scores" in ds.pred_instances:
                    ds.pred_instances = ds.pred_instances[ds.pred_instances.bbox_scores > bbox_thr]
        ...
        if pose_based_nms:
            for ds in data_samples:
                ...
                kept_indices = nearby_joints_nms(
                    [dict(keypoints=kpts[i], score=scores[i]) for i in range(len(kpts))],
