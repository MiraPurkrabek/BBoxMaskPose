...
from mmpose.apis import _track_by_iou, _track_by_oks, collate_pose_sequence, convert_keypoint_definition, extract_pose_sequence
...
class Pose3DInferencer(BaseMMPoseInferencer):
    ...
    preprocess_kwargs: set = {"bbox_thr", "nms_thr", "bboxes", "use_oks_tracking", "tracking_thr", "disable_norm_pose_2d"}
    ...
    def __init__(
        ...
        self.pose2d_model = Pose2DInferencer(
            pose2d_model if pose2d_model else "human", pose2d_weights, device, scope, det_model, det_weights, det_cat_ids
        )
    ...
    def preprocess_single(
        ...
        results_pose2d = next(
            self.pose2d_model(input, bbox_thr=bbox_thr, nms_thr=nms_thr, bboxes=bboxes, merge_results=False, return_datasamples=True)
        )["predictions"]

        for ds in results_pose2d:
            ds.pred_instances.set_field((ds.pred_instances.bboxes[..., 2:] - ds.pred_instances.bboxes[..., :2]).prod(-1), "areas")
        ...
        for result in results_pose2d:
            track_id, self._buffer["results_pose2d_last"], _ = _track(result, self._buffer["results_pose2d_last"], tracking_thr)
            if track_id == -1:
        ...
        for ds in results_pose2d_converted:
            ds.pred_instances.keypoints = self._keypoint_converter(ds.pred_instances.keypoints)
        self._buffer["pose_est_results_list"].append(results_pose2d_converted)

        # extract and pad input pose2d sequence
        pose_results_2d = self._pose_seq_extractor(self._buffer["pose_est_results_list"], frame_idx=index if self._video_input else 0)
        causal = self.cfg.test_dataloader.dataset.get("causal", False)
        ...
        pose_sequences_2d = collate_pose_sequence(pose_results_2d_copy, True, target_idx)
        ...
        for i, pose_seq in enumerate(pose_sequences_2d):
            ...
            data_info["data_samples"].set_field(img_path, "img_path", field_type="metainfo")
    ...
    def forward(self, inputs: Union[dict, tuple], disable_rebase_keypoint: bool = False):
        ...
        for idx, pose_lift_res in enumerate(pose_lift_results):
            ...
            pose_lift_res.track_id = pose_est_results_converted[idx].get("track_id", 1e4)
            ...
            if keypoint_scores.ndim == 3:
                pose_lift_results[idx].pred_instances.keypoint_scores = np.squeeze(keypoint_scores, axis=1)
            if keypoints.ndim == 4:
    ...
    def visualize(
        ...
        for single_input, pred in zip(inputs, preds):
            ...
            if vis_out_dir:
                img_name = os.path.basename(pred.metainfo["img_path"]) if "img_path" in pred.metainfo else None
