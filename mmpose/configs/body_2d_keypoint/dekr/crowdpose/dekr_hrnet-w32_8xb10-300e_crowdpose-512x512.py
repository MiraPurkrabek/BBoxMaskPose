...
param_scheduler = [
    ...
    dict(type="MultiStepLR", begin=0, end=300, milestones=[200, 260], gamma=0.1, by_epoch=True),
]
...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(type="Pretrained", checkpoint="https://download.openmmlab.com/mmpose/" "pretrain_models/hrnet_w32-36af842e.pth"),
    ),
    neck=dict(
    ...
    head=dict(
        ...
        rescore_cfg=dict(
            ...
            norm_indexes=(0, 1),
            init_cfg=dict(
                type="Pretrained", checkpoint="https://download.openmmlab.com/mmpose/" "pretrain_models/kpt_rescore_crowdpose-300c7efe.pth"
            ),
        ),
    ),
    test_cfg=dict(multiscale_test=False, flip_test=True, nms_dist_thr=0.05, shift_heatmap=True, align_corners=False),
)
...
val_pipeline = [
    dict(type="LoadImage"),
    dict(type="BottomupResize", input_size=codec["input_size"], size_factor=32, resize_mode="expand"),
    dict(
        type="PackPoseInputs",
        meta_keys=(
            "id",
