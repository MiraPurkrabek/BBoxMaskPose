...
from mmpose.datasets import BottomupRandomChoiceResize, BottomupRandomCrop, CocoDataset, LoadImage, PackPoseInputs, RandomFlip
from mmpose.evaluation import CocoMetric
from mmpose.models import BottomupPoseEstimator, ChannelMapper, EDPoseHead, PoseDataPreprocessor, ResNet
...
param_scheduler = [
    ...
    dict(type=MultiStepLR, begin=0, end=140, milestones=[33, 45], gamma=0.1, by_epoch=True),
]
...
model = dict(
    ...
    data_preprocessor=dict(
        type=PoseDataPreprocessor, mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], bgr_to_rgb=True, pad_size_divisor=1
    ),
    backbone=dict(
        type=ResNet,
        depth=50,
    ...
    head=dict(
        ...
        encoder=dict(
            ...
            layer_cfg=dict(  # DeformableDetrTransformerEncoderLayer
                self_attn_cfg=dict(  # MultiScaleDeformableAttention
                    embed_dims=256, num_heads=8, num_levels=4, num_points=4, batch_first=True
                ),
                ffn_cfg=dict(embed_dims=256, feedforward_channels=2048, num_fcs=2, ffn_drop=0.0),
            ),
        ),
        decoder=dict(
            num_layers=6,
            ...
            layer_cfg=dict(  # DeformableDetrTransformerDecoderLayer
                self_attn_cfg=dict(embed_dims=256, num_heads=8, batch_first=True),  # MultiheadAttention
        ...
        positional_encoding=dict(num_pos_feats=128, temperatureH=20, temperatureW=20, normalize=True),
        denosing_cfg=dict(
    ...
    dict(
        ...
        transforms=[
            ...
            [
                ...
                dict(type=BottomupRandomCrop, crop_type="absolute_range", crop_size=(384, 600), allow_negative_crop=True),
                dict(
                    type=BottomupRandomChoiceResize,
                    scales=[
                        (480, 1333),
...
val_pipeline = [
    dict(type=LoadImage),
    dict(type=BottomupRandomChoiceResize, scales=[(800, 1333)], keep_ratio=True, backend="pillow"),
    dict(
        type=PackPoseInputs,
        meta_keys=(
            "id",
