...
model = dict(
    ...
    head=dict(
        ...
        loss=dict(
            ...
            losses=[dict(type="SmoothL1Loss", use_target_weight=True), dict(type="KeypointMSELoss", use_target_weight=True)],
        ),
        decoder=codec,
    ),
    test_cfg=dict(
    ...
    init_cfg=dict(
        type="Pretrained", checkpoint="https://download.openmmlab.com/mmpose/" "pretrain_models/td-hm_res50_8xb64-210e_coco-256x192.pth"
    ),
)
...
# evaluators
val_evaluator = dict(type="CocoMetric", ann_file=f"{data_root}annotations/person_keypoints_val2017.json")
