...
# codec settings
codec = dict(type="SimCCLabel", input_size=input_size, sigma=(6.0, 6.93), simcc_split_ratio=2.0, normalize=False, use_dark=False)
...
backend_args = dict(
    backend="petrel", path_mapping=dict({f"{data_root}": "s3://openmmlab/datasets/", f"{data_root}": "s3://openmmlab/datasets/"})
)
...
aic_halpe26 = [(0, 6), (1, 8), (2, 10), (3, 5), (4, 7), (5, 9), (6, 12), (7, 14), (8, 16), (9, 11), (10, 13), (11, 15), (12, 17), (13, 18)]
...
dataset_coco = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=coco_halpe26)],
)
...
dataset_aic = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=aic_halpe26)],
)
...
dataset_crowdpose = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=crowdpose_halpe26)],
)
...
dataset_mpii = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=mpii_halpe26)],
)
...
dataset_jhmdb = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=jhmdb_halpe26)],
)
...
dataset_halpe = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=halpe_halpe26)],
)
...
dataset_posetrack = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=posetrack_halpe26)],
)
...
val_aic = dict(
    ...
    data_prefix=dict(img="pose/ai_challenge/ai_challenger_keypoint" "_validation_20170911/keypoint_validation_images_20170911/"),
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=aic_halpe26)],
)
...
val_ochuman = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=ochuman_halpe26)],
)
