...
param_scheduler = [
    ...
    dict(type="MultiStepLR", begin=0, end=210, milestones=[170, 190, 200], gamma=0.1, by_epoch=True),
]
...
codec = [dict(type="MegviiHeatmap", input_size=(192, 256), heatmap_size=(48, 64), kernel_size=kernel_size) for kernel_size in kernel_sizes]
...
model = dict(
    ...
    head=dict(
        ...
        loss=[dict(type="KeypointMSELoss", use_target_weight=True, loss_weight=0.25)] * 3
...
# evaluators
val_evaluator = dict(type="CocoMetric", ann_file=data_root + "annotations/person_keypoints_val2017.json", nms_mode="none")
