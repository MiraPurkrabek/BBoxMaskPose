...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type="Pretrained",
            prefix="backbone.",
            checkpoint="https://download.openmmlab.com/mmpose/" "pretrain_models/td-hm_res50_8xb64-210e_coco-256x192.pth",
        ),
    ),
    neck=dict(type="GlobalAveragePooling"),
    head=dict(type="RLEHead", in_channels=2048, num_joints=17, loss=dict(type="RLELoss", use_target_weight=True), decoder=codec),
    test_cfg=dict(
        flip_test=True,
        shift_coords=True,
    ),
...
# evaluators
val_evaluator = dict(type="CocoMetric", ann_file=f"{data_root}annotations/person_keypoints_val2017.json", score_mode="bbox_rle")
