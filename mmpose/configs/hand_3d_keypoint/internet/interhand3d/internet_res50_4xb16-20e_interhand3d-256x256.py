...
# learning policy
param_scheduler = [dict(type="MultiStepLR", begin=0, end=20, milestones=[15, 17], gamma=0.1, by_epoch=True)]

auto_scale_lr = dict(base_batch_size=128)

# hooks
default_hooks = dict(
    checkpoint=dict(type="CheckpointHook", interval=1, save_best="MPJPE_all", rule="less", max_keep_ckpts=1),
    logger=dict(type="LoggerHook", interval=20),
)

# codec settings
codec = dict(
    type="Hand3DHeatmap", image_size=[256, 256], root_heatmap_size=64, heatmap_size=[64, 64, 64], sigma=2.5, max_bound=255, depth_size=64
)
...
# evaluators
val_evaluator = [dict(type="InterHandMetric", modes=["MPJPE", "MRRPE", "HandednessAcc"])]
