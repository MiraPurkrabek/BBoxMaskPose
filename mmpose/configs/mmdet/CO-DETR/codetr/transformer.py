...
class Transformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed):
        ...
        out_dec = self.decoder(query=target, key=memory, value=memory, key_pos=pos_embed, query_pos=query_embed, key_padding_mask=mask)
...
class CoDeformableDetrTransformer(DeformableDetrTransformer):
    ...
    def forward_aux(
        ...
        **kwargs,
        ...
        init_reference_out = reference_points
        if self.num_co_heads > 0:
            pos_trans_out = self.aux_pos_trans_norm[head_idx](self.aux_pos_trans[head_idx](self.get_proposal_pos_embed(topk_coords_unact)))
            query_pos, query = torch.split(pos_trans_out, c, dim=2)
            if self.with_coord_feat:
