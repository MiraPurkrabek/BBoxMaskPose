...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_embedding=dict(type="AssociativeEmbeddingLoss", pull_weight=0.10, push_weight=0.10),
...
train_dataloader = dict(batch_size=6, num_workers=3, batch_sampler=None, dataset=dict(pipeline=train_pipeline))
val_dataloader = dict(dataset=dict(pipeline=test_pipeline))
test_dataloader = val_dataloader

# optimizer
optim_wrapper = dict(type="OptimWrapper", optimizer=dict(type="Adam", lr=0.0005), clip_grad=dict(max_norm=35, norm_type=2))
...
param_scheduler = [
    ...
    dict(type="MultiStepLR", begin=0, end=max_epochs, by_epoch=True, milestones=[180], gamma=0.1),
]
...
tta_model = dict(type="DetTTAModel", tta_cfg=dict(nms=dict(type="soft_nms", iou_threshold=0.5, method="gaussian"), max_per_img=100))
...
tta_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            [dict(type="LoadAnnotations", with_bbox=True)],
            [dict(type="PackDetInputs", meta_keys=("img_id", "img_path", "ori_shape", "img_shape", "flip", "flip_direction", "border"))],
        ],
    ),
]
