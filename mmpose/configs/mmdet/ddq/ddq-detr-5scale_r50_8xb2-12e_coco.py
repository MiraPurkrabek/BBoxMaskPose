...
model = dict(
    ...
    encoder=dict(
        num_layers=6,
        layer_cfg=dict(
            self_attn_cfg=dict(embed_dims=256, num_levels=5, dropout=0.0),  # 0.1 for DeformDETR
    ...
    decoder=dict(
        ...
        layer_cfg=dict(
            ...
            cross_attn_cfg=dict(embed_dims=256, num_levels=5, dropout=0.0),  # 0.1 for DeformDETR
            ffn_cfg=dict(embed_dims=256, feedforward_channels=2048, ffn_drop=0.0),  # 1024 for DeformDETR
        ),  # 0.1 for DeformDETR
        post_norm_cfg=None,
    ),
    positional_encoding=dict(num_feats=128, normalize=True, offset=0.0, temperature=20),  # -0.5 for DeformDETR  # 10000 for DeformDETR
    bbox_head=dict(
        type="DDQDETRHead",
        num_classes=80,
