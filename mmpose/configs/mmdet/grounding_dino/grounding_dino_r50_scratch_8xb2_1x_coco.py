...
model = dict(
    ...
    dn_cfg=dict(  # TODO: Move to model.train_cfg ?
        label_noise_scale=0.5, box_noise_scale=1.0, group_cfg=dict(dynamic=True, num_groups=None, num_dn_queries=100)  # 0.4 for DN-DETR
...
train_dataloader = dict(dataset=dict(filter_cfg=dict(filter_empty_gt=False), pipeline=train_pipeline, return_classes=True))
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(custom_keys={"absolute_pos_embed": dict(decay_mult=0.0), "backbone": dict(lr_mult=0.1)}),
)
# learning policy
max_epochs = 12
