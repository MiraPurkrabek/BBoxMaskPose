...
model = dict(
    ...
    track_head=dict(type="Mask2FormerTrackHead", in_channels=[192, 384, 768, 1536], num_queries=200),
    init_cfg=dict(
        type="Pretrained",
...
custom_keys.update({f"backbone.stages.{stage_id}.downsample.norm": backbone_norm_multi for stage_id in range(len(depths) - 1)})
# optimizer
