...
model = dict(
    ...
    track_head=dict(
        ...
        embed_head=dict(type="RoIEmbedHead", num_fcs=2, roi_feat_size=7, in_channels=256, fc_out_channels=1024),
        train_cfg=dict(
            assigner=dict(
                type="MaxIoUAssigner", pos_iou_thr=0.5, neg_iou_thr=0.5, min_pos_iou=0.5, match_low_quality=True, ignore_iof_thr=-1
            ),
            sampler=dict(type="RandomSampler", num=128, pos_fraction=0.25, neg_pos_ub=-1, add_gt_as_proposals=True),
            pos_weight=-1,
            debug=False,
        ),
    ),
...
# optimizer
optim_wrapper = dict(
    type="OptimWrapper", optimizer=dict(type="SGD", lr=0.00125, momentum=0.9, weight_decay=0.0001), clip_grad=dict(max_norm=35, norm_type=2)
)
