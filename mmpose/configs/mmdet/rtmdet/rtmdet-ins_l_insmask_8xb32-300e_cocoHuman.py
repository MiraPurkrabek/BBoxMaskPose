...
_base_ = [
    ...
    "../_base_/datasets/coco_human_instance.py",
    "./rtmdet_tta.py",
]
...
model = dict(
    ...
    test_cfg=dict(
        nms_pre=1000, min_bbox_size=0, score_thr=0.05, nms=dict(type="nms", iou_threshold=0.6), max_per_img=100, mask_thr_binary=0.5
    ),
)
...
train_dataloader = dict(batch_size=BATCH_SIZE, num_workers=10, batch_sampler=None, pin_memory=True, dataset=dict(pipeline=train_pipeline))
val_dataloader = dict(batch_size=BATCH_SIZE // 2, num_workers=10, dataset=dict(pipeline=test_pipeline))
...
train_cfg = dict(max_epochs=max_epochs, val_interval=interval, dynamic_intervals=[(max_epochs - stage2_num_epochs, 1)])
