...
model = dict(
    backbone=dict(depths=[2, 2, 18, 2, 1], num_heads=[4, 8, 16, 32, 64], strides=(4, 2, 2, 2, 2), out_indices=(1, 2, 3, 4)),
    neck=dict(in_channels=[256, 512, 1024, 2048]),
    bbox_head=dict(anchor_generator=dict(type="MlvlPointGenerator", offset=0, strides=[8, 16, 32, 64])),
)
...
train_pipeline = [
    ...
    dict(type="RandomResize", scale=(2560, 2560), ratio_range=(0.1, 2.0), keep_ratio=True),
    dict(type="RandomCrop", crop_size=(1280, 1280)),
    dict(type="YOLOXHSVRandomAug"),
    dict(type="RandomFlip", prob=0.5),
    dict(type="Pad", size=(1280, 1280), pad_val=dict(img=(114, 114, 114))),
    dict(type="CachedMixUp", img_scale=(1280, 1280), ratio_range=(1.0, 1.0), max_cached_images=20, pad_val=(114, 114, 114)),
...
train_dataloader = dict(batch_size=16, num_workers=20, dataset=dict(pipeline=train_pipeline))
...
tta_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            [
                dict(type="Pad", size=(1920, 1920), pad_val=dict(img=(114, 114, 114))),
            ],
            [dict(type="LoadAnnotations", with_bbox=True)],
            [
                dict(
