...
data_preprocessor = dict(
    type="DetDataPreprocessor", mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], bgr_to_rgb=True, pad_size_divisor=1
)
model = dict(
    ...
    train_cfg=dict(
        assigner=dict(type="MaxIoUAssigner", pos_iou_thr=0.5, neg_iou_thr=0.5, min_pos_iou=0.0, ignore_iof_thr=-1, gt_max_assign_all=False),
    ...
    test_cfg=dict(nms_pre=1000, nms=dict(type="nms", iou_threshold=0.45), min_bbox_size=0, score_thr=0.02, max_per_img=200),
)
...
train_pipeline = [
    ...
    dict(type="Expand", mean=data_preprocessor["mean"], to_rgb=data_preprocessor["bgr_to_rgb"], ratio_range=(1, 4)),
...
param_scheduler = [
    ...
    dict(type="CosineAnnealingLR", begin=0, T_max=max_epochs, end=max_epochs, by_epoch=True, eta_min=0),
]

# optimizer
optim_wrapper = dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.015, momentum=0.9, weight_decay=4.0e-5))
