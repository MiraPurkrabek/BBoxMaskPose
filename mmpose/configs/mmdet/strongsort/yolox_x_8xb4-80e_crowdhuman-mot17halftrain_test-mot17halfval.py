...
train_pipeline = [
    ...
    dict(type="RandomAffine", scaling_ratio_range=(0.1, 2), border=(-img_scale[0] // 2, -img_scale[1] // 2), bbox_clip_border=False),
    dict(type="MixUp", img_scale=img_scale, ratio_range=(0.8, 1.6), pad_val=114.0, bbox_clip_border=False),
...
default_hooks = dict(checkpoint=dict(interval=1, max_keep_ckpts=5))  # only keep latest 5 checkpoints
...
# evaluator
val_evaluator = dict(ann_file=data_root + "annotations/half-val_cocoformat.json", format_only=False)
