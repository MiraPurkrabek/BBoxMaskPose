...
model = dict(
    ...
    data_preprocessor=dict(
        type="DetDataPreprocessor", mean=img_norm_cfg["mean"], std=img_norm_cfg["std"], bgr_to_rgb=img_norm_cfg["to_rgb"], pad_mask=True
    ),
    backbone=dict(
        type="ResNet",
        depth=50,
    ...
    train_cfg=dict(
        assigner=dict(type="MaxIoUAssigner", pos_iou_thr=0.5, neg_iou_thr=0.4, min_pos_iou=0.0, ignore_iof_thr=-1, gt_max_assign_all=False),
    ...
    test_cfg=dict(
        nms_pre=1000, min_bbox_size=0, score_thr=0.05, mask_thr=0.5, iou_thr=0.5, top_k=200, max_per_img=100, mask_thr_binary=0.5
    ),
)
# dataset settings
train_pipeline = [
...
train_dataloader = dict(batch_size=8, num_workers=4, batch_sampler=None, dataset=dict(pipeline=train_pipeline))
...
param_scheduler = [
    ...
    dict(type="MultiStepLR", begin=0, end=max_epochs, by_epoch=True, milestones=[20, 42, 49, 52], gamma=0.1),
]

# optimizer
optim_wrapper = dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=1e-3, momentum=0.9, weight_decay=5e-4))
