...
model = dict(
    ...
    neck=dict(type="YOLOV3Neck", num_scales=3, in_channels=[320, 96, 32], out_channels=[96, 96, 96]),
    bbox_head=dict(
        type="YOLOV3Head",
        num_classes=80,
...
val_evaluator = dict(type="CocoMetric", ann_file=data_root + "annotations/instances_val2017.json", metric="bbox", backend_args=backend_args)
...
# optimizer
optim_wrapper = dict(
    type="OptimWrapper", optimizer=dict(type="SGD", lr=0.003, momentum=0.9, weight_decay=0.0005), clip_grad=dict(max_norm=35, norm_type=2)
)
