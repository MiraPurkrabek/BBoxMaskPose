...
# codec settings
codec = dict(type="SimCCLabel", input_size=input_size, sigma=(4.9, 5.66), simcc_split_ratio=2.0, normalize=False, use_dark=False)
...
model = dict(
    ...
    head=dict(
        ...
        gau_cfg=dict(
            hidden_dims=256, s=128, expansion_factor=2, dropout_rate=0.0, drop_path=0.0, act_fn="SiLU", use_rel_bias=False, pos_enc=False
        ),
        loss=dict(
            type="KLDiscretLoss",
...
train_pipeline_stage2 = [
    ...
    dict(type="RandomFlip", direction="horizontal"),
    dict(type="RandomHalfBody"),
    dict(type="RandomBBoxTransform", shift_factor=0.0, scale_factor=[0.5, 1.5], rotate_factor=90),
    dict(type="TopdownAffine", input_size=codec["input_size"]),
    dict(
        type="Albumentation",
        transforms=[
...
aic_coco133 = [(0, 6), (1, 8), (2, 10), (3, 5), (4, 7), (5, 9), (6, 12), (7, 14), (8, 16), (9, 11), (10, 13), (11, 15)]

crowdpose_coco133 = [(0, 5), (1, 6), (2, 7), (3, 8), (4, 9), (5, 10), (6, 11), (7, 12), (8, 13), (9, 14), (10, 15), (11, 16)]
...
halpe_coco133 = (
    [(i, i) for i in range(17)] + [(20, 17), (21, 20), (22, 18), (23, 21), (24, 19), (25, 22)] + [(i, i - 3) for i in range(26, 136)]
)
...
humanart_coco133 = [(i, i) for i in range(17)] + [(17, 99), (18, 120), (19, 17), (20, 20)]
...
dataset_aic = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=aic_coco133)],
)
...
dataset_crowdpose = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=crowdpose_coco133)],
)
...
dataset_jhmdb = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=jhmdb_coco133)],
)
...
dataset_halpe = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=halpe_coco133)],
)
...
dataset_posetrack = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=posetrack_coco133)],
)
...
dataset_humanart = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=humanart_coco133)],
)
...
face_pipeline = [
    ...
    dict(type="RandomBBoxTransform", shift_factor=0.0, scale_factor=[1.5, 2.0], rotate_factor=0),
]
...
wflw_coco133 = (
    ...
    + [(60, 59), (61, 60), (63, 61), (64, 62), (65, 63), (67, 64), (68, 65), (69, 66), (71, 67), (72, 68), (73, 69), (75, 70)]
...
dataset_wflw = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=wflw_coco133), *face_pipeline],
)
...
dataset_300w = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=mapping_300w_coco133), *face_pipeline],
)
...
dataset_cofw = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=cofw_coco133), *face_pipeline],
)
...
lapa_coco133 = (
    ...
    + [(66, 59), (67, 60), (69, 61), (70, 62), (71, 63), (73, 64), (75, 65), (76, 66), (78, 67), (79, 68), (80, 69), (82, 70)]
...
dataset_lapa = dict(
    ...
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=lapa_coco133), *face_pipeline],
)
