...
class UBody3dDataset(BaseMocapDataset):
    ...
    def __init__(self, multiple_target: int = 0, multiple_target_step: int = 0, seq_step: int = 1, pad_video_seq: bool = False, **kwargs):
    ...
    def get_sequence_indices(self) -> List[List[int]]:
        ...
        if self.multiple_target:
            for _, _img_ids in sorted(video_frames.items()):
                ...
                seqs_from_video = [_ann_ids[i : (i + self.multiple_target) : _step] for i in range(0, n_frame, self.multiple_target_step)][
            ...
            for _, _img_ids in sorted(video_frames.items()):
                ...
                if self.pad_video_seq:
                    ...
                    for i in range(n_frame):
                        ...
                        sequence_indices.append([_ann_ids[0]] * pad_left + _ann_ids[start:end:_step] + [_ann_ids[-1]] * pad_right)
                else:
    ...
    def _load_annotations(self):
        ...
        for i, _ann_ids in enumerate(self.sequence_indices):
            ...
            assert len(_ann_ids) == (expected_num_frames), f"Expected `frame_ids` == {expected_num_frames}, but " f"got {len(_ann_ids)} "
