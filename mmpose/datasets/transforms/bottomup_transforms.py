...
class BottomupRandomAffine(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if "keypoints" in results:
            ...
            if self.clip_border:
                results["keypoints"], results["keypoints_visible"] = keypoint_clip_border(
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        for i, (_w, _h) in enumerate(input_sizes):
            ...
            if self.use_udp:
                ...
                center = np.array([img_w / 2, img_h / 2], dtype=np.float32)
                scale = np.array(
                    [img_w * padded_input_size[0] / actual_input_size[0], img_h * padded_input_size[1] / actual_input_size[1]],
                    dtype=np.float32,
                )
            ...
            if results.get("keypoints", None) is not None:
                ...
                if self.bbox_clip_border:
                    ...
                    kpt_outside = np.logical_or.reduce(
                        (keypoints_outside_x, keypoints_outside_y, keypoints_outside_width, keypoints_outside_height)
                    )
...
class BottomupRandomChoiceResize(BaseTransform):
    ...
    def _resize_img(self, results: dict) -> None:
        ...
        if self.keep_ratio:
            ...
            img, scale_factor = imrescale(results["img"], self.scale, interpolation="bilinear", return_scale=True, backend=self.backend)
            ...
            img, w_scale, h_scale = imresize(results["img"], self.scale, interpolation="bilinear", return_scale=True, backend=self.backend)
