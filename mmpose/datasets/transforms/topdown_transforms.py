...
class TopdownAffine(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        assert results["bbox_center"].shape[0] == 1, (
            "Top-down heatmap only supports single instance. Got invalid " f'shape of bbox_center {results["bbox_center"].shape}.'
        )
        ...
        if isinstance(results["img"], list):
            results["img"] = [cv2.warpAffine(img, warp_mat, warp_size, flags=cv2.INTER_LINEAR) for img in results["img"]]
        else:
