...
def parse_args():
    ...
    parser.add_argument("pose_estimator_config", type=str, default=None, help="Config file for the 1st stage 2D pose estimator")
    parser.add_argument("pose_estimator_checkpoint", type=str, default=None, help="Checkpoint file for the 1st stage 2D pose estimator")
    parser.add_argument("pose_lifter_config", help="Config file for the 2nd stage pose lifter model")
    parser.add_argument("pose_lifter_checkpoint", help="Checkpoint file for the 2nd stage pose lifter model")
    ...
    parser.add_argument(
        "--output-root", type=str, default="", help="Root of the output video file. " "Default not saving the visualization video."
    )
...
def process_one_image(
    ...
    pose_lift_results = inference_pose_lifter_model(
        pose_lifter, pose_seq_2d, image_size=visualize_frame.shape[:2], norm_pose_2d=norm_pose_2d
    )
...
def main():
    ...
    pose_estimator = init_model(args.pose_estimator_config, args.pose_estimator_checkpoint, device=args.device.lower())

    assert isinstance(pose_estimator, TopdownPoseEstimator), 'Only "TopDown"' "model is supported for the 1st stage (2D pose detection)"
    ...
    pose_lifter = init_model(args.pose_lifter_config, args.pose_lifter_checkpoint, device=args.device.lower())

    assert isinstance(pose_lifter, PoseLifter), 'Only "PoseLifter" model is supported for the 2nd stage ' "(2D-to-3D lifting)"
    ...
    if input_type == "image":
        ...
        while video.isOpened():
            ...
            pose_est_results, pose_est_results_list, pred_3d_instances, next_id = process_one_image(
                args=args,
                detector=detector,
                frame=frame,
                frame_idx=frame_idx,
            ...
            if args.save_predictions:
                # save prediction results
                pred_instances_list.append(dict(frame_id=frame_idx, instances=split_instances(pred_3d_instances)))
            ...
            if save_output:
                ...
                if video_writer is None:
                    ...
                    video_writer = cv2.VideoWriter(output_file, fourcc, fps, (frame_vis.shape[1], frame_vis.shape[0]))
    ...
    if args.save_predictions:
        with open(args.pred_save_path, "w") as f:
            json.dump(dict(meta_info=pose_lifter.dataset_meta, instance_info=pred_instances_list), f, indent="\t")
