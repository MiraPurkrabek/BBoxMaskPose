...
model = dict(
    ...
    bbox_head=dict(in_channels=64, feat_channels=64, share_conv=False, exp_on_reg=False, use_depthwise=True, num_classes=1),
...
train_pipeline = [
    ...
    dict(type="CachedMosaic", img_scale=(input_shape, input_shape), pad_val=114.0, max_cached_images=20, random_pop=False),
    dict(type="RandomResize", scale=(input_shape * 2, input_shape * 2), ratio_range=(0.5, 1.5), keep_ratio=True),
...
train_pipeline_stage2 = [
    dict(type="LoadImageFromFile"),
    dict(type="LoadAnnotations", with_bbox=True),
    dict(type="RandomResize", scale=(input_shape, input_shape), ratio_range=(0.5, 1.5), keep_ratio=True),
    dict(type="RandomCrop", crop_size=(input_shape, input_shape)),
    dict(type="YOLOXHSVRandomAug"),
    dict(type="RandomFlip", prob=0.5),
    dict(type="Pad", size=(input_shape, input_shape), pad_val=dict(img=(114, 114, 114))),
...
train_dataset = dict(
    ...
    datasets=[
        ...
        dict(
            ...
            data_prefix=dict(img="pose/Halpe/hico_20160224_det/images/train2015/"),  # noqa
        ),
    ],
    ignore_keys=[
        "CLASSES",
...
custom_hooks = [
    ...
    dict(type="PipelineSwitchHook", switch_epoch=280, switch_pipeline=train_pipeline_stage2),
]

val_evaluator = dict(
    type="CocoMetric", ann_file=data_root + "onehand10k/annotations/onehand10k_test.json", metric="bbox", format_only=False
)
