...
param_scheduler = [
    ...
    dict(type="CosineAnnealingLR", eta_min=0.0005, begin=5, T_max=285, end=285, by_epoch=True, convert_to_iter_based=True),
...
train_dataset = dict(
    ...
    dataset=dict(
        ...
        pipeline=[dict(type="LoadImageFromFile", backend_args=dict(backend="local")), dict(type="LoadAnnotations", with_bbox=True)],
...
val_evaluator = dict(type="CocoMetric", ann_file="data/coco/annotations/coco_face_val.json", metric="bbox")
test_evaluator = dict(type="CocoMetric", ann_file="data/coco/annotations/instances_val2017.json", metric="bbox")
max_epochs = 300
num_last_epochs = 15
interval = 10
base_lr = 0.01
custom_hooks = [
    ...
    dict(type="EMAHook", ema_type="ExpMomentumEMA", momentum=0.0001, strict_load=False, update_buffers=True, priority=49),
]
metainfo = dict(CLASSES=("person",), PALETTE=(220, 20, 60))
launcher = "pytorch"
