...
class InterHandMetric(BaseMetric):
    ...
    def __init__(
        self, modes: List[str] = ["MPJPE", "MRRPE", "HandednessAcc"], collect_device: str = "cpu", prefix: Optional[str] = None
    ) -> None:
        super().__init__(collect_device=collect_device, prefix=prefix)
        for mode in modes:
            if mode not in self.METRICS:
                raise ValueError("`mode` should be 'MPJPE', 'MRRPE', or " f"'HandednessAcc', but got '{mode}'.")
    ...
    def process(self, data_batch: Sequence[dict], data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if "MPJPE" in self.modes:
                ...
                pred_coords_cam = pixel_to_camera(pred_coords_cam, focal[0], focal[1], principal_pt[0], principal_pt[1])
            ...
            if "MRRPE" in self.modes:
                ...
                if gt_hand_type.all() and keypoints_visible[..., 20] and keypoints_visible[..., 41]:
                    rel_root_mask = np.array([True])

                    pred_left_root_coords = np.array(pred_coords[..., 41, :], dtype=np.float32)
                    pred_left_root_coords[..., 2] += abs_depth[0] + pred_root_depth
                    pred_left_root_coords = pixel_to_camera(pred_left_root_coords, focal[0], focal[1], principal_pt[0], principal_pt[1])

                    pred_right_root_coords = np.array(pred_coords[..., 20, :], dtype=np.float32)
                    pred_right_root_coords[..., 2] += abs_depth[0]
                    pred_right_root_coords = pixel_to_camera(pred_right_root_coords, focal[0], focal[1], principal_pt[0], principal_pt[1])
                    pred_rel_root_coords = pred_left_root_coords - pred_right_root_coords
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if "MPJPE" in self.modes:
            ...
            interacting_mask = np.concatenate([result["interacting_mask"] for result in results])
            ...
            metrics["MPJPE_interacting"] = keypoint_epe(pred_coords, gt_coords, interacting_mask)

        if "HandednessAcc" in self.modes:
            pred_hand_type = np.concatenate([result["pred_hand_type"] for result in results])
            gt_hand_type = np.concatenate([result["gt_hand_type"] for result in results])
            hand_type_mask = np.concatenate([result["hand_type_mask"] for result in results])
            acc = (pred_hand_type == gt_hand_type).all(axis=-1)
            metrics["HandednessAcc"] = np.mean(acc[hand_type_mask])

        if "MRRPE" in self.modes:
            pred_rel_root_coords = np.concatenate([result["pred_rel_root_coords"] for result in results])
            gt_rel_root_coords = np.concatenate([result["gt_rel_root_coords"] for result in results])
            rel_root_mask = np.array([result["rel_root_mask"] for result in results])
            metrics["MRRPE"] = keypoint_epe(pred_rel_root_coords, gt_rel_root_coords, rel_root_mask)
        return metrics
