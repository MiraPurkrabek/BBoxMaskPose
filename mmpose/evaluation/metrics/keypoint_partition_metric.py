    ...
    def __init__(
        ...
        if metric["type"] == "CocoMetric":
            if "ann_file" in metric:
                warnings.warn(
                    "KeypointPartitionMetric does not support the ann_file " "argument of CocoMetric, this argument will be ignored."
                )
        ...
        if metric["type"] == "PCKAccuracy":
            ...
            if norm_item == "torso" or "torso" in norm_item:
                warnings.warn(
                    "norm_item torso is used in JhmdbDataset, it may not be " "compatible with other datasets, use at your own risk."
                )

        # check NME arguments
        if metric["type"] == "NME":
            assert "norm_mode" in metric, "Missing norm_mode required by the NME metric."
            if metric["norm_mode"] != "use_norm_item":
                raise ValueError("NME norm_mode 'keypoint_distance' is incompatible with " "KeypointPartitionMetric.")

        # check partitions
        assert len(partitions) > 0, "There should be at least one partition."
        for partition_name, partition in partitions.items():
            assert isinstance(partition, Sequence), "Each partition should be a sequence."
            assert len(partition) > 0, "Each partition should have at least one element."
        ...
        for partition_name in partitions.keys():
            ...
            if "outfile_prefix" in _metric:
                _metric["outfile_prefix"] = _metric["outfile_prefix"] + "." + partition_name
        ...
        for data_sample in data_samples:
            for partition_name, keypoint_ids in self.partitions.items():
                ...
                if "raw_ann_info" in _data_sample:
                    ...
                    anns = raw_ann_info if isinstance(raw_ann_info, list) else [raw_ann_info]
                    for ann in anns:
                        if "keypoints" in ann:
