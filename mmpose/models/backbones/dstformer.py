...
class Attention(BaseModule):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0, mode="spatial"):
        super().__init__()
    ...
    def forward_temporal(self, q, k, v, seq_len=8):
        B, _, N, C = q.shape
        qt = q.reshape(-1, seq_len, self.num_heads, N, C).permute(0, 2, 3, 1, 4)  # (B, H, N, T, C)
        kt = k.reshape(-1, seq_len, self.num_heads, N, C).permute(0, 2, 3, 1, 4)  # (B, H, N, T, C)
        vt = v.reshape(-1, seq_len, self.num_heads, N, C).permute(0, 2, 3, 1, 4)  # (B, H, N, T, C)
...
class AttentionBlock(BaseModule):
    ...
    def __init__(
        ...
        self.attn_s = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, mode="spatial"
        )
        self.attn_t = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, mode="temporal"
        )
        ...
        self.mlp_s = nn.Sequential(nn.Linear(dim, mlp_hidden_dim), nn.GELU(), nn.Linear(mlp_hidden_dim, mlp_out_dim), nn.Dropout(drop))
        self.mlp_t = nn.Sequential(nn.Linear(dim, mlp_hidden_dim), nn.GELU(), nn.Linear(mlp_hidden_dim, mlp_out_dim), nn.Dropout(drop))
...
class DSTFormer(BaseBackbone):
    ...
    def __init__(
        ...
        self.spat_embed = nn.Parameter(torch.zeros(1, num_keypoints, feat_size))
        ...
        if self.att_fuse:
            self.attn_regress = nn.ModuleList([nn.Linear(feat_size * 2, 2) for i in range(depth)])
            for i in range(depth):
