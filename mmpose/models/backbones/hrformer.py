...
class LocalWindowSelfAttention(BaseModule):
    ...
    def forward(self, x, H, W, **kwargs):
        ...
        if self.with_pad_mask and pad_h > 0 and pad_w > 0:
            ...
            pad_mask = pad(pad_mask, [0, 0, pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2], value=-float("inf"))
...
class CrossFFN(BaseModule):
    ...
    def __init__(
        ...
        self.dw3x3 = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, groups=hidden_features, padding=1)
        ...
        for i in range(num_branches if self.multiscale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            build_conv_layer(self.conv_cfg, num_inchannels[j], num_inchannels[i], kernel_size=1, stride=1, bias=False),
...
class HRFormer(HRNet):
    ...
    def __init__(
        ...
        # stochastic depth
        depths = [extra[stage]["num_blocks"][0] * extra[stage]["num_modules"] for stage in ["stage2", "stage3", "stage4"]]
        ...
        super().__init__(extra, in_channels, conv_cfg, norm_cfg, norm_eval, with_cp, zero_init_residual, frozen_stages, init_cfg)
