    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        ...
        if stride != 1 or self.in_channels[branch_index] != num_channels[branch_index] * get_expansion(block):
            downsample = nn.Sequential(
                ...
                build_norm_layer(self.norm_cfg, num_channels[branch_index] * get_expansion(block))[1],
            )
                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            nn.Upsample(
                                scale_factor=2 ** (j - i), mode=self.upsample_cfg["mode"], align_corners=self.upsample_cfg["align_corners"]
                            ),
                        )
                    )
                elif j == i:
                    ...
                    for k in range(i - j):
                        if k == i - j - 1:
                            conv_downsamples.append(
                                nn.Sequential(
                                    build_conv_layer(
                                        self.conv_cfg, in_channels[j], in_channels[i], kernel_size=3, stride=2, padding=1, bias=False
                                    ),
                                    build_norm_layer(self.norm_cfg, in_channels[i])[1],
                                )
                            )
                            ...
                            conv_downsamples.append(
                                nn.Sequential(
                                    build_conv_layer(
                                        self.conv_cfg, in_channels[j], in_channels[j], kernel_size=3, stride=2, padding=1, bias=False
                                    ),
...
class HRNet(BaseBackbone):
    ...
    def __init__(
        ...
        self.upsample_cfg = self.extra.get("upsample", {"mode": "nearest", "align_corners": None})
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, multiscale_output=self.stage4_cfg.get("multiscale_output", False)
        )
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv_downsamples.append(
                        nn.Sequential(
                            build_conv_layer(self.conv_cfg, in_channels, out_channels, kernel_size=3, stride=2, padding=1, bias=False),
    ...
    def _make_layer(self, block, in_channels, out_channels, blocks, stride=1):
        ...
        if stride != 1 or in_channels != out_channels:
            downsample = nn.Sequential(
                build_conv_layer(self.conv_cfg, in_channels, out_channels, kernel_size=1, stride=stride, bias=False),
                build_norm_layer(self.norm_cfg, out_channels)[1],
            )
        ...
        for _ in range(1, blocks):
            layers.append(block(out_channels, out_channels, with_cp=self.with_cp, norm_cfg=self.norm_cfg, conv_cfg=self.conv_cfg))
