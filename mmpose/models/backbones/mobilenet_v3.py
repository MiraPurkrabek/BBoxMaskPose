    ...
    def __init__(
        ...
        for index in out_indices:
            if index not in range(-len(self.arch_settings[arch]), len(self.arch_settings[arch])):
                raise ValueError("the item in out_indices must in " f"range(0, {len(self.arch_settings[arch])}). " f"But received {index}")

        if frozen_stages not in range(-1, len(self.arch_settings[arch])):
            raise ValueError("frozen_stages must be in range(-1, " f"{len(self.arch_settings[arch])}). " f"But received {frozen_stages}")
        self.arch = arch
