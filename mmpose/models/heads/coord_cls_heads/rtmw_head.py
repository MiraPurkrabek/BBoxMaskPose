...
class RTMWHead(BaseHead):
    ...
    def __init__(
        ...
        gau_cfg: ConfigType = dict(
            hidden_dims=256, s=128, expansion_factor=2, dropout_rate=0.0, drop_path=0.0, act_fn="ReLU", use_rel_bias=False, pos_enc=False
        ),
        ...
        self.mlp = nn.Sequential(ScaleNorm(flatten_dims), nn.Linear(flatten_dims, gau_cfg["hidden_dims"] // 2, bias=False))

        self.mlp2 = nn.Sequential(ScaleNorm(flatten_dims * ps**2), nn.Linear(flatten_dims * ps**2, gau_cfg["hidden_dims"] // 2, bias=False))
    ...
    def predict(
        ...
        if test_cfg.get("flip_test", False):
            ...
            _batch_pred_x_flip, _batch_pred_y_flip = flip_vectors(_batch_pred_x_flip, _batch_pred_y_flip, flip_indices=flip_indices)
