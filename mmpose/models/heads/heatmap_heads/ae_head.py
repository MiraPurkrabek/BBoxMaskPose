...
class AssociativeEmbeddingHead(HeatmapHead):
    ...
    def predict(self, feats: Features, batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        for scale_idx, _feats in enumerate(feats):
            if not flip_test:
                ...
                _heatmaps_flip = flip_heatmaps(_heatmaps_flip, flip_mode="heatmap", flip_indices=flip_indices, shift_heatmap=shift_heatmap)
                ...
                if scale_idx == 0:
                    _tags = aggregate_heatmaps([_tags_orig, _tags_flip], size=heatmap_size, align_corners=align_corners, mode="concat")
                else:
                    _tags = None
    ...
    def decode(self, batch_outputs: Union[Tensor, Tuple[Tensor]]) -> InstanceList:
        ...
        if self.decoder.support_batch_decoding:
            batch_keypoints, batch_scores, batch_instance_scores = _pack_and_call(batch_outputs, self.decoder.batch_decode)
