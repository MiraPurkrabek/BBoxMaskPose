...
import torch
from mmcv.cnn import ConvModule, DepthwiseSeparableConvModule, Linear, build_activation_layer, build_norm_layer
...
from mmpose.utils.typing import ConfigType, MultiConfig, OptConfigType, OptSampleList, Predictions
...
class PRM(nn.Module):
    ...
    def __init__(self, out_channels: int, norm_cfg: ConfigType = dict(type="BN")):
        ...
        self.bottom_path = nn.Sequential(
            ConvModule(self.out_channels, self.out_channels, kernel_size=1, stride=1, padding=0, norm_cfg=norm_cfg, inplace=False),
            DepthwiseSeparableConvModule(self.out_channels, 1, kernel_size=9, stride=1, padding=4, norm_cfg=norm_cfg, inplace=False),
            build_activation_layer(dict(type="Sigmoid")),
        )
...
class PredictHeatmap(nn.Module):
    ...
    def __init__(
        self, unit_channels: int, out_channels: int, out_shape: tuple, use_prm: bool = False, norm_cfg: ConfigType = dict(type="BN")
    ):
        ...
        self.conv_layers = nn.Sequential(
            ConvModule(unit_channels, unit_channels, kernel_size=1, stride=1, padding=0, norm_cfg=norm_cfg, inplace=False),
            ConvModule(unit_channels, out_channels, kernel_size=3, stride=1, padding=1, norm_cfg=norm_cfg, act_cfg=None, inplace=False),
        )
...
class MSPNHead(BaseHead):
    ...
    def __init__(
        ...
        if len(level_indices) != num_stages * num_units:
            raise ValueError(
                f"The length of level_indices({len(level_indices)}) did not " f"match `num_stages`({num_stages}) * `num_units`({num_units})"
            )
        ...
        if isinstance(loss, list) and len(loss) != num_stages * num_units:
            raise ValueError(
                f"The length of loss_module({len(loss)}) did not match " f"`num_stages`({num_stages}) * `num_units`({num_units})"
            )
    ...
    def forward(self, feats: Sequence[Sequence[Tensor]]) -> List[Tensor]:
        ...
        """
        out = []
        assert len(feats) == self.num_stages, f"The length of feature maps did not match the " f"`num_stages` in {self.__class__.__name__}"
        for feat in feats:
            assert len(feat) == self.num_units, f"The length of feature maps did not match the " f"`num_units` in {self.__class__.__name__}"
            for f in feat:
                assert f.shape[1] == self.unit_channels, (
                    f"The number of feature map channels did not match the " f"`unit_channels` in {self.__class__.__name__}"
                )
    ...
    def predict(
        self, feats: Union[MSMUFeatures, List[MSMUFeatures]], batch_data_samples: OptSampleList, test_cfg: OptConfigType = {}
    ) -> Predictions:
    ...
    def loss(self, feats: MSMUFeatures, batch_data_samples: OptSampleList, train_cfg: OptConfigType = {}) -> dict:
        ...
        keypoint_weights = torch.cat([d.gt_instance_labels.keypoint_weights for d in batch_data_samples])  # shape: [B*N, L, K]
        ...
        for i in range(self.num_stages * self.num_units):
            ...
            gt_heatmaps = torch.stack([d.gt_fields[self.level_indices[i]].heatmaps for d in batch_data_samples])
            loss_i = loss_func(msmu_pred_heatmaps[i], gt_heatmaps, keypoint_weights[:, self.level_indices[i]])
        ...
        _, avg_acc, _ = pose_pck_accuracy(
            output=to_numpy(msmu_pred_heatmaps[-1]), target=to_numpy(gt_heatmaps), mask=to_numpy(keypoint_weights[:, -1]) > 0
        )
