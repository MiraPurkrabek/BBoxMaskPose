...
class IntegralRegressionHead(BaseHead):
    ...
    def __init__(
        ...
        if num_deconv != 0:
            ...
            if final_layer is not None:
                cfg = dict(type="Conv2d", in_channels=in_channels, out_channels=num_joints, kernel_size=1)
    ...
    def predict(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if test_cfg.get("flip_test", False):
            ...
            _batch_coords_flip = flip_coordinates(
                _batch_coords_flip, flip_indices=flip_indices, shift_coords=test_cfg.get("shift_coords", True), input_size=input_size
            )
            _batch_heatmaps_flip = flip_heatmaps(
                _batch_heatmaps_flip, flip_mode="heatmap", flip_indices=flip_indices, shift_heatmap=test_cfg.get("shift_heatmap", False)
            )
        ...
        for _k in keys:
            ...
            if self.simplebaseline_head is not None:
                if k_parts[0] == "conv_layers":
                    k_new = prefix + "simplebaseline_head.deconv_layers." + ".".join(k_parts[1:])
