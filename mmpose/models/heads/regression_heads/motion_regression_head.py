...
class MotionRegressionHead(BaseHead):
    ...
    def __init__(
        ...
        loss: ConfigType = dict(type="MSELoss", use_target_weight=True),
        ...
        self.pre_logits = nn.Sequential(OrderedDict([("fc", nn.Linear(in_channels, embedding_size)), ("act", nn.Tanh())]))
        self.fc = nn.Linear(embedding_size, out_channels) if embedding_size > 0 else nn.Identity()
    ...
    def predict(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if test_cfg.get("flip_test", False):
            ...
            _batch_coords_flip = torch.stack(
                [
                    flip_coordinates(
                        _batch_coord_flip, flip_indices=flip_indices, shift_coords=test_cfg.get("shift_coords", True), input_size=(1, 1)
                    )
        ...
        if camera_param is not None:
            ...
            w = torch.stack([torch.empty((0), dtype=torch.float32) for _ in batch_data_samples])
    ...
    def loss(self, inputs: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: ConfigType = {}) -> dict:
        ...
        lifting_target_label = torch.stack([d.gt_instance_labels.lifting_target_label for d in batch_data_samples])
        lifting_target_weight = torch.stack([d.gt_instance_labels.lifting_target_weight for d in batch_data_samples])

        # calculate losses
        losses = dict()
        loss = self.loss_module(pred_outputs, lifting_target_label, lifting_target_weight.unsqueeze(-1))

        losses.update(loss_pose3d=loss)

        # calculate accuracy
        mpjpe_err = keypoint_mpjpe(pred=to_numpy(pred_outputs), gt=to_numpy(lifting_target_label), mask=to_numpy(lifting_target_weight) > 0)

        mpjpe_pose = torch.tensor(mpjpe_err, device=lifting_target_label.device)
