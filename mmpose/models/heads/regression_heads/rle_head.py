...
class RLEHead(BaseHead):
    ...
    def predict(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if test_cfg.get("flip_test", False):
            ...
            _batch_coords_flip = flip_coordinates(
                self.forward(_feats_flip), flip_indices=flip_indices, shift_coords=test_cfg.get("shift_coords", True), input_size=input_size
            )
    ...
    def loss(self, inputs: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: ConfigType = {}) -> dict:
        ...
        # calculate losses
        losses = dict()
        loss = self.loss_module(pred_coords, pred_sigma, keypoint_labels, keypoint_weights.unsqueeze(-1))
