...
class TrajectoryRegressionHead(BaseHead):
    ...
    def __init__(
        ...
        num_joints: int,
        loss: ConfigType = dict(type="MPJPELoss", use_target_weight=True),
    ...
    def loss(self, inputs: Union[Tensor, Tuple[Tensor]], batch_data_samples: OptSampleList, train_cfg: ConfigType = {}) -> dict:
        ...
        # calculate losses
        losses = dict()
        loss = self.loss_module(pred_outputs, lifting_target_label, trajectory_weights.unsqueeze(-1))

        losses.update(loss_traj=loss)

        # calculate accuracy
        mpjpe_err = keypoint_mpjpe(pred=to_numpy(pred_outputs), gt=to_numpy(lifting_target_label), mask=to_numpy(trajectory_weights) > 0)

        mpjpe_traj = torch.tensor(mpjpe_err, device=lifting_target_label.device)
