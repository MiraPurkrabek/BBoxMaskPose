...
from .transformers.deformable_detr_layers import DeformableDetrTransformerDecoderLayer, DeformableDetrTransformerEncoder
...
class EDPoseDecoder(BaseModule):
    ...
    def __init__(
        ...
        self.layers = ModuleList([DeformableDetrTransformerDecoderLayer(**self.layer_cfg) for _ in range(self.num_layers)])
        self.norm = nn.LayerNorm(self.embed_dims)

        self.ref_point_head = FFN(self.query_dim // 2 * self.embed_dims, self.embed_dims, self.embed_dims, 2)
        ...
        self.kpt_index = [x for x in range(self.num_group * (self.num_keypoints + 1)) if x % (self.num_keypoints + 1) != 0]
    ...
    def forward(
        ...
        valid_ratios: Tensor,
        humandet_attn_mask: Tensor,
        human2pose_attn_mask: Tensor,
        **kwargs,
    ) -> Tuple[Tensor]:
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            query_sine_embed = self.get_proposal_pos_embed(reference_points_input[:, :, 0, :])  # nq, bs, 256*2
            ...
            if layer_id == self.num_box_decoder_layers - 1:
                ...
                new_reference_points_for_box = torch.gather(
                    new_reference_points[effect_num_dn:], 0, topk_proposals.unsqueeze(-1).repeat(1, 1, 4)
                )
                new_output_for_box = torch.gather(output[effect_num_dn:], 0, topk_proposals.unsqueeze(-1).repeat(1, 1, self.embed_dims))
            ...
            if layer_id >= self.num_box_decoder_layers:
                ...
                outputs_unsig = (
                    ref_before_sigmoid[effect_num_dn:].index_select(0, torch.tensor(self.kpt_index, device=output.device)).clone()
                )
                ...
                new_reference_points_norm = torch.cat(
                    (new_reference_points_for_box_norm.unsqueeze(1), new_reference_points_for_keypoint.view(-1, self.num_keypoints, bs, 4)),
                    dim=1,
                ).flatten(0, 1)
        ...
        reference_points = [itm_refpoint.transpose(0, 1) for itm_refpoint in intermediate_reference_points]

        return decoder_outputs, reference_points

    @staticmethod
    def get_proposal_pos_embed(pos_tensor: Tensor, temperature: int = 10000, num_pos_feats: int = 128) -> Tensor:
...
class EDPoseOutHead(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        _class_embed = nn.Linear(self.embed_dims, self.num_classes, bias=(not cls_no_bias))
        if not cls_no_bias:
            prior_prob = 0.01
        ...
        if dec_pred_bbox_embed_share:
            ...
            box_embed_layerlist = [copy.deepcopy(_bbox_embed) for i in range(num_pred_layer)]
        if dec_pred_class_embed_share:
            class_embed_layerlist = [_class_embed for i in range(num_pred_layer)]
        else:
            class_embed_layerlist = [copy.deepcopy(_class_embed) for i in range(num_pred_layer)]

        if num_keypoints == 17:
            if dec_pred_pose_embed_share:
                pose_embed_layerlist = [_pose_embed for i in range(num_pred_layer - num_box_decoder_layers + 1)]
            else:
                pose_embed_layerlist = [copy.deepcopy(_pose_embed) for i in range(num_pred_layer - num_box_decoder_layers + 1)]
        else:
            if dec_pred_pose_embed_share:
                pose_embed_layerlist = [_pose_embed for i in range(num_pred_layer - num_box_decoder_layers)]
            else:
                pose_embed_layerlist = [copy.deepcopy(_pose_embed) for i in range(num_pred_layer - num_box_decoder_layers)]

        pose_hw_embed_layerlist = [_pose_hw_embed for i in range(num_pred_layer - num_box_decoder_layers)]
    ...
    def forward(
        ...
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_cls_embed, layer_hs) in enumerate(
            zip(references[:-1], self.bbox_embed, self.class_embed, hidden_states)
        ):
            if dec_lid < self.num_box_decoder_layers:
                ...
                layer_hs_bbox_norm = layer_hs[:, effec_dn_num:, :][:, 0 :: (self.num_keypoints + 1), :]
                bs = layer_ref_sig.shape[0]
                ref_before_sigmoid_bbox_dn = layer_ref_sig[:, :effec_dn_num, :]
                ref_before_sigmoid_bbox_norm = layer_ref_sig[:, effec_dn_num:, :][:, 0 :: (self.num_keypoints + 1), :]
                ...
                layer_outputs_unsig_norm = layer_delta_unsig_norm + inverse_sigmoid(ref_before_sigmoid_bbox_norm)
        ...
        kpt_index = [x for x in range(self.num_group * (self.num_keypoints + 1)) if x % (self.num_keypoints + 1) != 0]
        for dec_lid, (layer_ref_sig, layer_hs) in enumerate(zip(references[:-1], hidden_states)):
            if dec_lid < self.num_box_decoder_layers:
                assert isinstance(layer_hs, torch.Tensor)
                bs = layer_hs.shape[0]
                layer_res = layer_hs.new_zeros((bs, self.num_queries, self.num_keypoints * 3))
                outputs_keypoints_list.append(layer_res)
            else:
                bs = layer_ref_sig.shape[0]
                layer_hs_kpt = layer_hs[:, effec_dn_num:, :].index_select(1, torch.tensor(kpt_index, device=layer_hs.device))
                delta_xy_unsig = self.pose_embed[dec_lid - self.num_box_decoder_layers](layer_hs_kpt)
                layer_ref_sig_kpt = layer_ref_sig[:, effec_dn_num:, :].index_select(1, torch.tensor(kpt_index, device=layer_hs.device))
                ...
                xyv = xyv.sigmoid()
                layer_res = xyv.reshape((bs, self.num_group, self.num_keypoints, 3)).flatten(2, 3)
        ...
        if self.refine_queries_num > 0 and dn_mask_dict is not None:
            outputs_class, outputs_coord_list, outputs_keypoints_list = self.dn_post_process2(
                outputs_class, outputs_coord_list, outputs_keypoints_list, dn_mask_dict
            )
...
class EDPoseHead(TransformerHead):
    ...
    def __init__(
        ...
        super().__init__(
            encoder=encoder, decoder=decoder, out_head=out_head, positional_encoding=positional_encoding, num_queries=num_queries
        )
        ...
        if self.as_two_stage:
            ...
            if dec_pred_class_embed_share and dec_pred_bbox_embed_share:
                ...
                self.enc_out_bbox_embed = copy.deepcopy(self.out_head.bbox_embed[0])

            if dec_pred_class_embed_share and dec_pred_bbox_embed_share:
                self.enc_out_class_embed = self.out_head.class_embed[0]
            else:
                self.enc_out_class_embed = copy.deepcopy(self.out_head.class_embed[0])
    ...
    def pre_transformer(self, img_feats: Tuple[Tensor], batch_data_samples: OptSampleList = None) -> Tuple[Dict]:
        ...
        mask_flatten = []
        spatial_shapes = []
        for lvl, (feat, mask, pos_embed) in enumerate(zip(img_feats, mlvl_masks, mlvl_pos_embeds)):
        ...
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=feat_flatten.device)  # (num_level, 2)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))  # (num_level)
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in mlvl_masks], 1)  # (bs, num_level, 2)

        if self.refine_queries_num > 0 or batch_data_samples is not None:
            input_query_label, input_query_bbox, humandet_attn_mask, human2pose_attn_mask, mask_dict = self.prepare_for_denosing(
                batch_data_samples, device=img_feats[0].device
            )
        else:
            assert batch_data_samples is None
            input_query_bbox = input_query_label = humandet_attn_mask = human2pose_attn_mask = mask_dict = None
    ...
    def forward_encoder(self, img_feats: Tuple[Tensor], batch_data_samples: OptSampleList = None) -> Dict:
    ...
    def pre_decoder(
        self, memory: Tensor, memory_mask: Tensor, spatial_shapes: Tensor, input_query_bbox: Tensor, input_query_label: Tensor
    ) -> Tuple[Dict, Dict]:
        ...
        if self.as_two_stage:
            ...
            query_undetach = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.embed_dims))
            query = query_undetach.detach()

            if input_query_bbox is not None:
                reference_points = torch.cat([input_query_bbox, topk_coords_unact], dim=1).sigmoid()
    ...
    def forward_decoder(
        ...
        """
        decoder_in, head_in = self.pre_decoder(memory, memory_mask, spatial_shapes, input_query_bbox, input_query_label)
        ...
        decoder_outputs_dict = dict(hidden_states=inter_states, references=references, mask_dict=mask_dict)
    ...
    def forward_out_head(
        ...
        """Forward function."""
        out = self.out_head(hidden_states, references, mask_dict, hidden_states_enc, referens_enc, batch_data_samples)
        return out
    ...
    def predict(self, feats: Features, batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if test_cfg.get("flip_test", False):
            assert NotImplementedError("flip_test is currently not supported " "for EDPose. Please set `model.test_cfg.flip_test=False`")
        else:
            pred_logits, pred_boxes, pred_keypoints = self.forward(feats, batch_data_samples)  # (B, K, D)

            pred = self.decode(input_shapes, pred_logits=pred_logits, pred_boxes=pred_boxes, pred_keypoints=pred_keypoints)
        return pred

    def decode(self, input_shapes: np.ndarray, pred_logits: Tensor, pred_boxes: Tensor, pred_keypoints: Tensor):
        ...
        if self.data_decoder is None:
            raise RuntimeError(
                f"The data decoder has not been set in \
        ...
        for input_shape, pred_logit, pred_bbox, pred_kpts in zip(input_shapes, pred_logits, pred_boxes, pred_keypoints):

            bboxes, keypoints, keypoint_scores = self.data_decoder.decode(input_shape, pred_logit, pred_bbox, pred_kpts)

            # pack outputs
            preds.append(InstanceData(keypoints=keypoints, keypoint_scores=keypoint_scores, bboxes=bboxes))

        return preds

    def gen_encoder_output_proposals(self, memory: Tensor, memory_mask: Tensor, spatial_shapes: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    def prepare_for_denosing(self, targets: OptSampleList, device):
        ...
        for idx, (gt_boxes_i, gt_labels_i, gt_keypoint_i) in enumerate(zip(gt_boxes, gt_labels, gt_keypoints)):
            ...
            if num_gt_i > 0:
                ...
                gt_labels_expand_i = torch.ones(refine_queries_num, dtype=torch.int64, device=device) * int(self.num_classes)
                gt_keypoints_expand_i = torch.rand(refine_queries_num, self.num_keypoints * 3, device=device)
        ...
        if self.denosing_cfg["dn_box_noise_scale"] > 0:
            ...
            knwon_boxes_expand += torch.mul((torch.rand_like(knwon_boxes_expand) * 2 - 1.0), diff) * self.denosing_cfg["dn_box_noise_scale"]
        ...
        if "group2group" in self.denosing_cfg["dn_attn_mask_type_list"]:
            ...
            for matchj in range(self.num_group * (self.num_keypoints + 1)):
                ...
                ej = (matchj // group_bbox_kpt + 1) * group_bbox_kpt
                if sj > 0:
                    attn_mask2[:, :, refine_queries_num:, refine_queries_num:][:, :, matchj, :sj] = True
                if ej < self.num_group * (self.num_keypoints + 1):
                    attn_mask2[:, :, refine_queries_num:, refine_queries_num:][:, :, matchj, ej:] = True

            for match_x in range(self.num_group * (self.num_keypoints + 1)):
                if match_x % group_bbox_kpt == 0:
                    attn_mask2[:, :, refine_queries_num:, refine_queries_num:][:, :, match_x, kpt_index] = False
            ...
            for idx, (gt_boxes_i, gt_labels_i) in enumerate(zip(gt_boxes, gt_labels)):
                ...
                for matchi in range(refine_queries_num):
                    ...
                    if ei < refine_queries_num:
                        attn_mask2[idx, :, matchi, ei:refine_queries_num] = True
    ...
    def loss(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: OptConfigType = {}) -> dict:
        ...
        assert NotImplementedError("the training of EDPose has not been " "supported. Please stay tuned for further update.")
