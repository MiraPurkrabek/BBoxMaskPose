...
class KeypointMSELoss(nn.Module):
    ...
    def _get_mask(self, target: Tensor, target_weights: Optional[Tensor], mask: Optional[Tensor]) -> Optional[Tensor]:
        ...
        if mask is not None:
            ...
            assert mask.ndim == target.ndim and all(d_m == d_t or d_m == 1 for d_m, d_t in zip(mask.shape, target.shape)), (
...
class AdaptiveWingLoss(nn.Module):
    ...
    def criterion(self, pred, target):
        ...
        losses = torch.where(
            delta < self.theta, self.omega * torch.log(1 + torch.pow(delta / self.epsilon, self.alpha - target)), A * delta - C
        )
...
class FocalHeatmapLoss(KeypointMSELoss):
    ...
    def __init__(
        self, alpha: int = 2, beta: int = 4, use_target_weight: bool = False, skip_empty_channel: bool = False, loss_weight: float = 1.0
    ):
...
class MLECCLoss(nn.Module):
    ...
    def __init__(self, reduction: str = "mean", mode: str = "log", use_target_weight: bool = False, loss_weight: float = 1.0):
        super().__init__()
        assert reduction in ("mean", "sum", "none"), f"`reduction` should be either 'mean', 'sum', or 'none', " f"but got {reduction}"
        assert mode in ("linear", "square", "log"), f"`mode` should be either 'linear', 'square', or 'log', " f"but got {mode}"
...
class OKSHeatmapLoss(nn.Module):
    ...
    def forward(
        ...
        if per_pixel:
            ...
            loss = (
                oks_minus_weight * oks.sum(dim=(2, 3)) + self.smoothing_weight * max_gradient + self.gaussian_weight * mse.mean(dim=(2, 3))
            )
        else:
            max_gradient, _ = gradient.reshape((B, K, H * W)).max(dim=-1)
            loss = (
