...
from mmpose.utils.typing import ConfigType, InstanceList, OptConfigType, Optional, OptMultiConfig, OptSampleList, PixelDataList, SampleList
    ...
    def causal(self):
        ...
        if hasattr(self.backbone, "causal"):
            ...
            raise AttributeError(
                "A PoseLifter's backbone should have " 'the bool attribute "causal" to indicate if' "it performs causal inference."
            )
        ...
        if self.test_cfg.get("flip_test", False):
            ...
            _feats_flip = self.extract_feat(
                torch.stack(
                    [
                        flip_coordinates(
                            _input, flip_indices=flip_indices, shift_coords=self.test_cfg.get("shift_coords", True), input_size=(1, 1)
                        )
