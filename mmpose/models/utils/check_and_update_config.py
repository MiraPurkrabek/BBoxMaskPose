...
def process_input_transform(
    ...
    if input_transform == "resize_concat":
        ...
        raise ValueError(f"model.head get invalid value for argument " f"input_transform: {input_transform}")


def process_extra_field(extra: Dict, head_new: Dict, head_deleted_dict: Dict, head_append_dict: Dict, neck_new: Dict) -> None:
    ...
    if "final_conv_kernel" in extra:
        ...
        if kernel_size > 1:
            ...
            head_append_dict["final_layer"] = f"dict(kernel_size={kernel_size}, " f"padding={padding})"
        else:
...
def process_has_final_layer(has_final_layer: bool, head_new: Dict, head_deleted_dict: Dict, head_append_dict: Dict) -> None:
...
def check_and_update_config(neck: Optional[ConfigType], head: ConfigType) -> Tuple[Optional[Dict], Dict]:
    ...
    process_input_transform(input_transform, head, head_new, head_deleted_dict, head_append_dict, neck_new, input_index, align_corners)

    if "extra" in head:
        extra = head_new.pop("extra")
        process_extra_field(extra, head_new, head_deleted_dict, head_append_dict, neck_new)

    if "has_final_layer" in head:
        has_final_layer = head_new.pop("has_final_layer")
        process_has_final_layer(has_final_layer, head_new, head_deleted_dict, head_append_dict)
...
@master_only
def display_modifications(head_deleted_dict: Dict, head_append_dict: Dict, neck: Dict) -> None:
    ...
    old_model_info, new_model_info = build_model_info(head_deleted_dict, head_append_dict, neck)
...
def build_model_info(head_deleted_dict: Dict, head_append_dict: Dict, neck: Dict) -> Tuple[str, str]:
