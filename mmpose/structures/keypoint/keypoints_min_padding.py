...
def find_min_padding_exact(bbox, kpts, aspect_ratio=3 / 4, bbox_format="xywh"):
    """
    Find the minimum padding factor to make all keypoints lie inside the bbox.

    Args:
        bbox (np.ndarray): Bounding box in the specified format.
        kpts (np.ndarray): Keypoints array of shape (K, 2) or (K, 3).
        aspect_ratio (float or None): Target aspect ratio applied before padding check.
            Pass None to skip aspect-ratio adjustment.
        bbox_format (str): Bounding box format, either 'xywh' or 'xyxy'.

    Returns:
        np.ndarray: Per-keypoint padding factors (shape (K,)). Values <= 0 indicate
            invisible keypoints (visibility <= 0).
    """
...
def fix_bbox_aspect_ratio(bbox, aspect_ratio=3 / 4, padding=1.25, bbox_format="xywh"):
    """
    Adjust bounding box dimensions to match a target aspect ratio with optional padding.

    Args:
        bbox (np.ndarray): Bounding box or array of boxes.
        aspect_ratio (float): Target width/height ratio. Defaults to 3/4.
        padding (float): Scale factor applied to the adjusted box. Defaults to 1.25.
        bbox_format (str): Input format, either 'xywh' or 'xyxy'. Defaults to 'xywh'.

    Returns:
        np.ndarray: Adjusted bounding box(es) in the same format as input.
    """
    assert bbox_format.lower() in ["xywh", "xyxy"], f"Invalid bbox format {bbox_format}. Only 'xyxy' or 'xywh' are supported."
