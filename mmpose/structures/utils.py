...
def merge_data_samples(data_samples: List[PoseDataSample]) -> PoseDataSample:
    ...
    if "pred_fields" in data_samples[0] and "heatmaps" in data_samples[0].pred_fields:
        reverted_heatmaps = [
            revert_heatmap(data_sample.pred_fields.heatmaps, data_sample.input_center, data_sample.input_scale, data_sample.ori_shape)
    ...
    if "gt_fields" in data_samples[0] and "heatmaps" in data_samples[0].gt_fields:
        reverted_heatmaps = [
            revert_heatmap(data_sample.gt_fields.heatmaps, data_sample.input_center, data_sample.input_scale, data_sample.ori_shape)
            for data_sample in data_samples
        ]
...
def revert_heatmap(heatmap, input_center, input_scale, img_shape):
    ...
    warp_mat = get_warp_matrix(input_center.reshape((2,)), input_scale.reshape((2,)), rot=0, output_size=(hm_w, hm_h), inv=True)
