...
class PMPose:
    ...
    def __init__(
        ...
        if config_path is not None:
            ...
            raise ValueError(f"Unknown variant '{variant}'. Available: {list(DEFAULT_CONFIGS.keys())}")
        ...
        if from_pretrained:
            ...
            if pretrained_id not in PRETRAINED_URLS:
                raise ValueError(f"Unknown pretrained_id '{pretrained_id}'. " f"Available: {list(PRETRAINED_URLS.keys())}")
    ...
    def predict(
        ...
        pose_results = inference_topdown(self._model, img, bboxes=bboxes, masks=masks, bbox_format="xyxy")
        ...
        for result in pose_results:
            ...
            # Extract visibility if available, otherwise use scores as a proxy
            try:
                kpt_visibiliteies = pred_instances.keypoints_visible.reshape(K, 1)  # (K, 1)
            except AttributeError:
                kpt_visibiliteies = np.copy(kpt_scores)

            # Extract presence probabilities if available, otherwise use scores as a proxy
            try:
                kpt_presence = pred_instances.keypoints_probs.reshape(K, 1)  # (K, 1)
            except AttributeError:
                kpt_presence = np.copy(kpt_scores)

            # Combine into (K, 3) format
            kpts_with_scores = np.concatenate([kpts, kpt_scores], axis=1)
            keypoints_list.append(kpts_with_scores)
            scores_list.append(kpt_scores)
            visibilities_list.append(kpt_visibiliteies)
            probabilities_list.append(kpt_presence)
        ...
        keypoint_visibilities = np.stack(visibilities_list, axis=0) if visibilities_list else np.zeros((0, K, 1))
        keypoint_presence = np.stack(probabilities_list, axis=0) if probabilities_list else np.zeros((0, K, 1))
        ...
        return keypoints, keypoint_presence, keypoint_visibilities, heatmaps
    ...
    def visualize(
        ...
        keypoints_17 = keypoints[
            :, :17, :
        ]  # Assuming COCO format for visualization. For now, the visualization supports only 17 keypoints.
